...
void Cascade_Star_Sigma_Star_Sigma_Width_Input(){
  ...
  TFile fileOutput1("/shared/storage/physhad/JLab/mn688/Dibaryon_Paper/Output/dsss_Width_Cascade_Star_M_Sigma_Star_M_To_Sigma_Mol_Output_1B_21022022_01.root","recreate");
  // TFile fileOutput1("../Output/dsss_Width_Cascade_Star_M_Sigma_Star_M_To_Sigma_Output_1B_21022022_01.root","recreate");

  /////////////////////////////////////////////////////////////////////////////////
  /////  Create the histograms     //////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////////////////

  TH2F* hbranching_ratio=new TH2F("hbranching_ratio","",200,1,1.5,1000,0,1);
  ...
  TH2F* hInv_Sigma_Star_M=new TH2F("hInv_Sigma_Star_M","Invariant mass of p and 2#pi^{-};M(p 2#pi^{-}) [GeV];Width",300,1,1.5,200,0.0,0.1);
  ...
  TH2F* hInv_Cascade_Star_M=new TH2F("hInv_Cascade_Star_M","Invariant mass of p, 2#pi^{-} and #pi^{0};M(p 2#pi^{-} #pi^{0}) [GeV];Width",300,1.3,1.8,200,0.0,0.1);
  ...
  TLorentzVector *Sigma_Star_M_Sigma, *Sigma_Star_M_Pim, *Cascade_Star_M_Cascade, *Cascade_Star_M_Pim;
  // Invariant masses
  ...
  // Masses of particles
  Double_t Masses_1[4] = {1.19745, 0.13957, 1.31486, 0.13957}; // Masses of particles from 1st decay vertex
  ...
  for (Long64_t i=0;i<nevents;){
    ...
    Sigma_Star_M_Sigma=Vertex_1.GetDecay(0);
    ...
    Inv_Sigma_Star_M = (TLorentzVector)*Sigma_Star_M_Sigma + (TLorentzVector)*Sigma_Star_M_Pim; // Invariant mass for Sigma^{*-}
    Inv_dsss = (TLorentzVector)*Cascade_Star_M_Pim + (TLorentzVector)*Cascade_Star_M_Cascade + (TLorentzVector)*Sigma_Star_M_Sigma + (TLorentzVector)*Sigma_Star_M_Pim; // Invariant mass for dsss^{--}
    ...
    q_Cascade_Star_M_Pim = sqrt(fabs(pow(E_Cascade_Star_M_Pim,2) - pow(Masses_1[3],2)));

    // Sigma Star Minus to Lambda Pion
    E_Sigma_Star_M_Lambda_Pim = (pow(Inv_Sigma_Star_M.M(),2) + pow(Masses_1[1],2) - pow(1.11568,2))/(2*Inv_Sigma_Star_M.M()); // Manually put in mass of sigma ground state because simulating sigma decay channel instead
    q_Sigma_Star_M_Lambda_Pim = sqrt(fabs(pow(E_Sigma_Star_M_Lambda_Pim,2) - pow(Masses_1[3],2)));

    // Sigma Star Minus to Sigma Pion
    E_Sigma_Star_M_Sigma_Pim = (pow(Inv_Sigma_Star_M.M(),2) + pow(Masses_1[1],2) - pow(Masses_1[0],2))/(2*Inv_Sigma_Star_M.M());
    q_Sigma_Star_M_Sigma_Pim = sqrt(fabs(pow(E_Sigma_Star_M_Sigma_Pim,2) - pow(Masses_1[1],2)));
    ...
    Cascade_Star_M_Width = (gamma_BW_Cascade_Star * (pow(q_Cascade_Star_M_Pim,3)) * pow(R,2)) / (1 + pow(R,2) * pow(q_Cascade_Star_M_Pim,2));
    Sigma_Star_M_Lambda_Width = (0.27 * (pow(q_Sigma_Star_M_Lambda_Pim,3)) * pow(R,2)) / (1 + pow(R,2) * pow(q_Sigma_Star_M_Lambda_Pim,2));
    Sigma_Star_M_Sigma_Width = (0.096 * (pow(q_Sigma_Star_M_Sigma_Pim,3)) * pow(R,2)) / (1 + pow(R,2) * pow(q_Sigma_Star_M_Sigma_Pim,2));
    ...
    D_Sigma_Star_M = (sqrt((m_Sigma_Star_M*Sigma_Star_M_Sigma_Width)/q_Sigma_Star_M_Sigma_Pim))*(1.0/(pow(Inv_Sigma_Star_M.M(),2) - pow(m_Sigma_Star_M,2) + comp_Sigma_Star_M));
    ...
    Width_Weight_noq = pow(FF,2) * gamma_R * (D_Sigma_Star_M * D_Cascade_Star_M).Rho2();
    ...
    hInv_dsss_M_Phs_WW_FF_noq->Fill(Inv_dsss.M(), Phasespace_Weight_1 * Width_Weight_noq); // All weights applied except phasespace and q factor

    // Binding energies of dss with various weights applied
    hBinding_dsss_M_Phs_WW_FF_noq->Fill(Binding_dsss, Phasespace_Weight_1 * Width_Weight_noq);
    ...
    hbranching_ratio->Fill(Inv_Sigma_Star_M.M(),Sigma_Star_M_Lambda_Width / Sigma_Star_M_Total_Width);
    i++;
  }

  cout<<0.0394 / hInv_Sigma_Star_M->GetBinContent(hInv_Sigma_Star_M->FindBin(1.3872))<<endl;
